...
boost new (Critter name type1 trait1)
    | new > trait1   = Critter name type1 new
    | otherwise = Critter name type1 trait1

----------------------------------------------------------------------
-- LabProblem #4: define the function combine (see the lab writeup)
----------------------------------------------------------------------
combine :: Animal -> Animal -> Animal
combine (Critter name1 typa1 traits1) (Critter name2 typa2 traits2)
    | typa1 >= typa2     = boost traits2 (Critter (name1 ++ name2) typa1 traits1)
    | otherwise     = boost traits1 (Critter (name1 ++ name2) typa2 traits2)